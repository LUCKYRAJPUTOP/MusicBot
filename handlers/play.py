...
async def play_(client: Client, message: Message):
    await message.reply_text("**RAJPUTBOT:** Sorry! I can only be used in groups. \nTry again in a group.")
...
async def play(client: Client, message_: Message):
    ...
    res = await message_.reply_text("**RAJPUTBOT:** 🔄 Processing...")

    if audio:
        if round(audio.duration / 60) > DURATION_LIMIT:
            raise DurationLimitError(
                f"**RAJPUTBOT:** Videos longer than {DURATION_LIMIT} minute(s) aren't allowed, the provided video is {audio.duration / 60} minute(s)"
            )
        ...
        if offset == None:
            await res.edit_text("**RAJPUTBOT:**❕ You did not give me anything to play.")
            return
    ...
    if is_playing:
        ...
        await res.edit_text(f"**RAJPUTBOT:** #️⃣ Queued at position {position}.")
    else:
        await res.edit_text("**RAJPUTBOT:** ▶️ Playing...")
